...
class ApiStrategy {
  ...
  ApiStrategy._internal() {
    if (_client == null) {
      ...
      _client!.interceptors.add(InterceptorsWrapper(onRequest:(options, handler) =>
          tokenInterceptor(options, handler)));
  ...
  Future<dynamic> tokenInterceptor(RequestOptions options, RequestInterceptorHandler handler) async {
    try {
      if (SharedPrefs.isContains(LOGINDATA)) {
        LoginModel loginModel = LoginModel.fromJson(SharedPrefs.getCustomObject(LOGINDATA));
        options.headers.addAll({"Authorization": "Bearer ${loginModel.accessToken}"});
      }
